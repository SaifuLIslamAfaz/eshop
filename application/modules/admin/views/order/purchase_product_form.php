...
$("#dynamic_row").append('<tr style="text-align:center"><td>' + (i + 1) + '</td><td><select onchange=" get_product_data(' + i + ')" id="product_id_' + i + '" name="p_id[]" class="select2  form-control"><option value="">Select Product</option><?php foreach ($product_list as $row) { ?><option value="<?=$row["pro_id"]; ?>"><?=$row["product_name"]; ?></option><?php } ?></select></td><td><input onchange="qty_update_calculation(' + i + ')" id="product_qty_' + i + '" class="form-control sell_cart_input" name="buy_qty[]" type="number" value="0"></td><td><div class="input-group icon_tag_input"><input name="price[]" id="price_id_' + i + '" name="price[]" readonly value="0" class="form-control align-right" type="number" ></div></td><td><div class="input-group icon_tag_input"><input id="product_price_qty_' + i + '" readonly value="0.00" class="form-control align-right subtotal" type="number" ></div></td><td><button class="rem_row btn btn-danger btn-xs" style="padding: 3px 10px;"><i class="fa fa-trash"></i></button></td></tr>');

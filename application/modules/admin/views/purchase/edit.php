...
<div class="card-body">
<form action="admin/purchaseUpdate/<?=$purchase_edit['id']?>" enctype="multipart/form-data" method="post" class="form form-horizontal striped-rows">
<div class="form-body">
...
<div class="row">
  <div class="col-xl-5 col-lg-6 col-md-12 mb-1">
    ...
    <select class="select2 form-control" id="id_h5_single" name="supplier_name">
      ...
      <?php foreach ($supp_list as $value) { ?>
        <option value="<?= $value['sup_id']; ?>" <?=$value['sup_id']==$purchase_edit['supp_id'] ? 'selected' : false ?>><?= $value['sup_name']; ?></option>
      <?php } ?>
    </select>
  </div>
  <div class="col-xl-3 col-lg-6 col-md-12 mb-1">
    <fieldset class="form-group">
      ...
      <input type="date" value="<?=$purchase_edit['date']?>" class="form-control" id="basicInput" name="date">
    </fieldset>
  </div>
  <div class="col-xl-4 col-lg-6 col-md-12 mb-1">
    <fieldset class="form-group">
      <label for="basicInput">Bill-No</label>
      <input type="number" value="<?=$purchase_edit['memo_no']?>" class="form-control" id="basicInput" name="bill_no">
...
<div class="row">
  <div class="col-xl-7 col-lg-6 col-md-12 mb-1">
    ...
    <span class="">
      <textarea class="form-control"  id="placeTextarea" rows="5" placeholder="Textarea with placeholder" name="notation"><?=$purchase_edit['notation']?></textarea>
    </span>
  </div>
  <div class="col-xl-5 col-lg-6 col-md-12 mb-1">
    <fieldset class="form-group">
    ...
    <img src="<?=base_url('uploads/purchases/'.$purchase_edit['file'])?>" width="120px" height="100px"/> 
    <input type="hidden" name="old_file" value="<?=$purchase_edit['file']?>">
  </div>
</div>
</div>
<!-- ================= table start ================ -->
...
<table class="table  table-bordered default-ordering table-responsive">
  ...
  <tbody id="dynamic_row">
       ...
       <?php foreach($purchase_products as $key=>$row):?>
    <tr style="text-align:center;">
      ...
      <td>
        <select onchange="get_product_data(0)" id="product_id_0" name="p_id[]" class="select2 form-control abcd" data-placeholder="Select a product">
          ...
          <?php foreach ($product_list as $value) { ?>
            <option value="<?=$value['pro_id']; ?>" <?php if($value['pro_id']==$row['product_id']) echo 'selected'?>><?=$value['product_name']; ?></option>
          <?php } ?>
        </select>
      </td>
            <td>
        <div class="input-group icon_tag_input abc">
          <input id="price_id_<?=$row['id']?>" name="price[]"  value="<?=$row['price']?>" class="form-control align-right " type="number">
        
        </div>
      </td>
      <td><input onchange="qty_update_calculation(<?=$row['id']?>)" name="buy_qty[]" id="product_qty_<?=$row['id']?>" class="form-control qty_css" type="number" value="<?=$row['qty']?>"></td>

      <td>
        <div class="input-group icon_tag_input abc">
          <input id="product_price_qty_<?=$row['id']?>" readonly value="<?=$row['price']*$row['qty']?>" class="form-control align-right subtotal" type="number">
      ...
      <td>
        ...
        <?php elseif($key>0):?>
         ...
         <a class="rem_row btn btn-danger btn-xs"  style="padding: 3px 10px;">
  ...
  <tr>
    ...
    <td>
      <div class="input-group icon_tag_input total_div">
        <input name="credit" id="total" readonly value="<?=$purchase_edit['total_price']?>" class="form-control align-right" type="number">
  ...
  <tr>
    ...
    <td>
      <div class="input-group icon_tag_input">
        <input name="debit" value="<?=$purchase_edit['paid_amount']?>" onkeyup="due_advance_calculation()" id="paid" class="form-control align-right" type="number">
  ...
  <tr>
    ...
    <td>
      <div class="input-group icon_tag_input">
        <input id="remain_val" readonly class="form-control align-right" type="number" value="<?=$purchase_edit['due_amount']?>">
  ...
  <tr>
    ...
    <td>
      <div class="input-group icon_tag_input">
        <input value="<?=$purchase_edit['supp_id']?>" name="unload_cost" class="form-control align-right" type="number">
...
<button id="type-info" type="submit" class="btn btn-primary col-sm-4">

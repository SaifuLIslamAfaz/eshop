	<!-- CSS
	============================================ -->
	<!-- Web Font --> 
	<link href="https://fonts.googleapis.com/css?family=Poppins:200i,300,300i,400,400i,500,500i,600,600i,700,700i,800,800i,900,900i&display=swap" rel="stylesheet">
	<!-- Bootstrap -->
	<link rel="stylesheet" href="frontassets/css/bootstrap.css">
	<!-- Magnific Popup -->
    <link rel="stylesheet" href="frontassets/css/magnific-popup.min.css">
	<!-- Font Awesome -->
    <link rel="stylesheet" href="frontassets/css/font-awesome.css">
	<!-- Fancybox -->
	<link rel="stylesheet" href="frontassets/css/jquery.fancybox.min.css">
	<!-- Themify Icons -->
    <link rel="stylesheet" href="frontassets/css/themify-icons.css">
	<!-- Nice Select CSS -->
    <link rel="stylesheet" href="frontassets/css/niceselect.css">
	<!-- Animate CSS -->
    <link rel="stylesheet" href="frontassets/css/animate.css">
	<!-- Flex Slider CSS -->
    <link rel="stylesheet" href="frontassets/css/flex-slider.min.css">
	<!-- Owl Carousel -->
    <link rel="stylesheet" href="frontassets/css/owl-carousel.css">
	<!-- Slicknav -->
    <link rel="stylesheet" href="frontassets/css/slicknav.min.css">
	<!-- Eshop StyleSheet -->
	<link rel="stylesheet" href="frontassets/css/reset.css">
	<link rel="stylesheet" href="frontassets/css/style.css">
    <link rel="stylesheet" href="frontassets/css/responsive.css">

</head>


  
